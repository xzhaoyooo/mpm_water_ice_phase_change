import taichi as ti
import math
from taichi.linalg import MatrixFreeCG, LinearOperator

ti.init(arch=ti.cpu)

GRID = 2
x = ti.field(dtype=ti.f32, shape=GRID * GRID)
b = ti.field(dtype=ti.f32, shape=GRID * GRID)

@ti.func
def flatten(i: int, j: int) -> int:
    return i * GRID + j

@ti.kernel
def init():
    for i, j in ti.ndrange(GRID, GRID):
        xl = i / (GRID - 1)
        yl = j / (GRID - 1)
        b[flatten(i, j)] = 1
        x[flatten(i, j)] = 0.0

@ti.kernel
def compute_Ax(v: ti.template(), mv: ti.template()): # pyright: ignore
    for i, j in ti.ndrange(GRID, GRID):
        l = v[flatten(i - 1, j)] if i - 1 >= 0 else 0.0
        r = v[flatten(i + 1, j)] if i + 1 <= GRID - 1 else 0.0
        t = v[flatten(i, j + 1)] if j + 1 <= GRID - 1 else 0.0
        b = v[flatten(i, j - 1)] if j - 1 >= 0 else 0.0
        # Avoid ill-conditioned matrix A
        mv[flatten(i, j)] = 20 * v[flatten(i, j)] - l - r - t - b

A = LinearOperator(compute_Ax)
init()
MatrixFreeCG(A, b, x, maxiter=10 * GRID * GRID, tol=1e-18, quiet=False)
print(x.to_numpy())